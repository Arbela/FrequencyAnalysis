...
namespace FrequencyAnalysis
{
    public class VideoAnalyzer : IVideoAnalyzer
    {
        ...
        public async Task<IEnumerable<DataPoint>> Analyze(string convertedImagesPath, bool verticalOnly = false, bool horizontalOnly = false)
        {
            string[] bmps = Directory.GetFiles(convertedImagesPath);

            IList<DataPoint> measuresBlur = new List<DataPoint>();
            ...
            {
                tasks.Add(Task.Run(() => measuresBlur.Add(new DataPoint(++i + 1, AnalyzeImage(bmp)))));
            }
